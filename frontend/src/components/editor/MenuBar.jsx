  ...
  const MenuBar = ({ editor }) => {
    ...
    return (
      <div className="border border-slate-300 rounded-lg p-5 sticky top-3 left-0 right-0 bg-white z-10 flex gap-0.5 flex-wrap">
        <button
          onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}  //chain:Create a command chain to call multiple commands at once.
          className={`editor-btn font-black ${
            editor.isActive("heading", { level: 1 }) && "active-editor-btn" //isActive: Returns if the currently selected node or mark is active.
          }`}
        >
          H1
        </button>
